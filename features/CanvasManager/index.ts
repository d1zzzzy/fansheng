...
class CanvasManager {
  ...
  constructor(container: HTMLElement, options: CanvasOptions = {}) {
    ...
    window.addEventListener('resize', () => this.resizeCanvas(container, options));
  }

  private resizeCanvas(container: HTMLElement, options: CanvasOptions) {
...
// 设置画布位置
function positionCanvas(canvas: HTMLCanvasElement, container: HTMLElement) {
...
function rotatePoint(point: Point, angle: number, rotateAt: Point): Point {
  const cosTheta = Math.cos(angle);
  const sinTheta = Math.sin(angle);
  return {
    x: cosTheta * (point.x - rotateAt.x) - sinTheta * (point.y - rotateAt.y) + rotateAt.x,
